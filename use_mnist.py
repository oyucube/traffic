...
import importlib
import socket
from mylib.my_functions import get_batch_mnist, draw_attention_mnist
import matplotlib
import importlib
matplotlib.use('Agg')
...
parser.add_argument("-d", "--data", type=str, default="mnist",
                    help="data")
parser.add_argument("-l", "--l", type=str, default="bn2",
...
if socket.gethostname() == "chainer":
    ...
    log_dir = "log/"
# load data
data_dir = data_dir + "mnistdata/"
dl = importlib.import_module("dataset." + args.data)
train_data = dl.MyDataset(data_dir)
val_data = dl.MyDataset(data_dir)
...
with chainer.function.no_backprop_mode(), chainer.using_config('train', False):
    x, t = get_batch_mnist(val_data, perm[0:test_b], 1)
...
for i in range(test_b):
    ...
    # print(l_list[1][i])
    # print(s_list[0][i])
    draw_attention_mnist(img, l_list, s_list, i, save=save_filename)
