...
#         place_after = np.random.rand(2) * (256 - size_after - 32) + (16 + size_after / 2) * np.ones(2)
        ...
        if place_crop[0] < 0 or place_crop[1] < 0 \
            ...
            # label file format
            # image_path, label
            f_label.write(str(out_id) + ".jpg, " + str(label) + ", " + str(size[0][0] / 256)
                          + ", " + str(position[0][0] + size[0][0] / 512)
                          + ", " + str(position[0][1] + size[0][0] / 512)
                          + ", " + str(size[1][0] / 256)
                          + ", " + str(position[1][0] + size[1][0] / 512)
                          + ", " + str(position[1][1] + size[1][0] / 512)
                          + "\n")
            return 1
    return 0
